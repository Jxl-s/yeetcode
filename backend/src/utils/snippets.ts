...
class Method {
    ...
    public toJavaScript(isConstructor = false) {
        ...
        if (this.name === '__init__') {
            ...
            snippet += `\nvar ${this.name} = function(${argList}) {`;
        } else {
...
export class SnippetsAlgo {
    ...
    private _makeJavaScript() {
        return this.method.toJavaScript(true);
    }
